        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <!-- SALIDAS (Admin, Gerencia, Mantenimiento) -->
                    @if(in_array(Auth::user()->role?->name, ['Administrador', 'Gerencia', 'Mantenimiento']))
                        <x-nav-link :href="route('material-outputs.index')" :active="request()->routeIs('material-outputs.*')">
                            {{ __('Registro de Salidas') }}
                        </x-nav-link>
                    @endif

                    <!-- ENTRADAS (Admin, Gerencia) -->
                    @if(in_array(Auth::user()->role?->name, ['Administrador', 'Gerencia']))
                        <x-nav-link :href="route('material-receptions.index')" :active="request()->routeIs('material-receptions.*')">
                            {{ __('Registro de Entradas') }}
                        </x-nav-link>
                    @endif
                    
                    <!-- (NUEVO) HAZMAT (Admin, Seguridad y Salud) -->
                    @if(in_array(Auth::user()->role?->name, ['Administrador', 'Seguridad y Salud']))
                        <x-nav-link :href="route('hazmat.index')" :active="request()->routeIs('hazmat.*')">
