        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <!-- Usamos la ruta 'update' y el método 'PATCH' -->
                    <form method="POST" action="{{ route('material-receptions.update', $recepcione) }}">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                            <div>
                                ...
                                <select id="material_type" name="material_type" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required>
                                    <option value="CONSUMIBLE" {{ old('material_type', $recepcione->material_type) == 'CONSUMIBLE' ? 'selected' : '' }}>CONSUMIBLE</option>
                                    <option value="SPARE_PART" {{ old('material_type', $recepcione->material_type) == 'SPARE_PART' ? 'selected' : '' }}>SPARE PART</option>
                                </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                            <div>
                                ...
                                <x-text-input id="item_number" class="block mt-1 w-full" type="text" name="item_number" :value="old('item_number', $recepcione->item_number)" />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                            <div>
                                ...
                                <x-text-input id="sap_confirmation" class="block mt-1 w-full" type="text" name="sap_confirmation" :value="old('sap_confirmation', $recepcione->sap_confirmation)" />
                                <x-input-error :messages="$errors->get('sap_confirmation')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="storage_location" :value="__('Ubicación de Almacenamiento (Llenar para completar)')" />
                                <x-text-input id="storage_location" class="block mt-1 w-full bg-yellow-50 dark:bg-yellow-900" type="text" name="storage_location" :value="old('storage_location', $recepcione->storage_location)" placeholder="Ej: Pasillo 4, Estante B" />
                        ...
                        <div class="block mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                            <label for="quality_certificate" class="inline-flex items-center">
                                <input id="quality_certificate" type="checkbox" value="1" name="quality_certificate" class="rounded border-gray-300 dark:border-gray-700 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:focus:ring-indigo-600 dark:focus:ring-offset-gray-800" {{ old('quality_certificate', $recepcione->quality_certificate) ? 'checked' : '' }}>
