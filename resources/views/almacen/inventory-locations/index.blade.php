    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Ubicaciones de Inventario') }}
            </h2>
            
            @if(Auth::user()->role->name === 'Administrador' || Auth::user()->role->name === 'Almacenista')
                <a href="{{ route('inventory-locations.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-4">
                <form method="GET" action="{{ route('inventory-locations.index') }}" class="flex gap-4">
                    <div class="flex-1">
                        <x-text-input id="search" name="search" type="text" class="block w-full" :value="$search" placeholder="Buscar por código, nombre o descripción..." />
                    </div>
                    <x-primary-button>
                        {{ __('Buscar') }}
                    </x-primary-button>
                    @if($search)
                        <a href="{{ route('inventory-locations.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-200 dark:bg-gray-700 border border-transparent rounded-md font-semibold text-xs text-gray-700 dark:text-gray-300 uppercase tracking-widest hover:bg-gray-300 dark:hover:bg-gray-600 transition ease-in-out duration-150">
                            {{ __('Limpiar') }}
                        </a>
                    @endif
                </form>
                                ...
                                @forelse($locations as $location)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('inventory-locations.edit', $location) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 mr-3">Editar</a>
                                            
                                            <form action="{{ route('inventory-locations.destroy', $location) }}" method="POST" class="inline-block" onsubmit="return confirm('¿Estás seguro de eliminar esta ubicación?');">
                                                @csrf
                                                @method('DELETE')
