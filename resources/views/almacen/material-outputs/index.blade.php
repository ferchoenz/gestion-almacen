<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ $viewDeleted ? __('Registro de Salidas CANCELADAS') : __('Registro de Salida de Materiales') }}
        </h2>
    </x-slot>
    ...
    <div class="py-12" x-data="{ showCancelModal: false, cancelUrl: '' }">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-gray-100 dark:bg-gray-700 p-4 rounded-t-lg border-b border-gray-200 dark:border-gray-600">
                <form method="GET" action="{{ route('material-outputs.index') }}">
                    @if($viewDeleted)
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th class="px-6 py-3">No. Item</th>
                                    <th class="px-6 py-3">Cant.</th>
                                    <th class="px-6 py-3">Recibió</th>
                                    @if($viewDeleted)
                            ...
                            <tbody>
                                ...
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <td class="px-6 py-4 whitespace-nowrap">{{ \Carbon\Carbon::parse($output->output_date)->format('d/m/Y') }}</td>
                                    <td class="px-6 py-4 font-bold">{{ $output->terminal->name }}</td>
                                    <!-- Clases break-words para ajustar -->
                                    <td class="px-6 py-4 font-medium text-gray-900 dark:text-white max-w-xs break-words">{{ $output->description }}</td>
                                    <td class="px-6 py-4">{{ $output->material_type }}</td>
                                    <td class="px-6 py-4">{{ $output->item_number ?? 'N/A' }}</td>
                                    <td class="px-6 py-4 font-bold">{{ $output->quantity }}</td>
                                    <td class="px-6 py-4 max-w-[150px] break-words">{{ $output->receiver_name }}</td>
                                    
                                    @if($viewDeleted)
                                        <td class="px-6 py-4 text-red-600 font-bold max-w-xs break-words">{{ $output->cancellation_reason }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $output->deleted_at->format('d/m/Y H:i') }}</td>
                                    @else
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if($output->status == 'COMPLETO')
                                            ...
                                            @elseif($output->status == 'PENDIENTE_SAP')
                                                <span class="bg-yellow-100 text-yellow-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-yellow-900 dark:text-yellow-300">PENDIENTE SAP</span>
                                            @else
                                                <span class="bg-gray-100 text-gray-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">PENDIENTE OT</span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 text-right whitespace-nowrap">
                                            <a href="{{ route('material-outputs.pdf', $output) }}" target="_blank" class="font-medium text-green-600 hover:underline mr-3">PDF</a>
                                            @if(Auth::user()->role?->name === 'Administrador')
                                                <a href="{{ route('material-outputs.edit', $output) }}" class="font-medium text-indigo-600 hover:underline mr-3">Editar</a>
                                                <button @click="showCancelModal = true; cancelUrl = '{{ route('material-outputs.destroy', $output) }}'" class="font-medium text-red-600 hover:underline">Cancelar</button>
                                            @endif
                                        </td>
                                    @endif
                                </tr>
